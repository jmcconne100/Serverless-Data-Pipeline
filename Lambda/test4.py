...
class FakeRelationalGenerator:
    """Parent class for relational multi-table dataset generation."""

    def __init__(self, filename_prefix="relational_db", config=None):
        self.filename_prefix = filename_prefix
        self.config = config if isinstance(config, CSVConfig) else CSVConfig()  # Ensure it's a CSVConfig object
        self.fake = Faker()
        self.tables = {}  # Stores table data

    def generate_data(self):
        """Child classes must define how tables are generated."""
        raise NotImplementedError("Child class must implement generate_data()")

    def export_relational_csv(self):
        """Exports relational tables to CSV files."""
        for table_name, rows in self.tables.items():
            filename = f"{self.filename_prefix}_{table_name}.csv"
...
class FakeHRDatabase(FakeRelationalGenerator):
    """Generates relational HR dataset with Employees, Departments, and Salaries."""

    def generate_data(self):
        """Generates HR-related relational datasets."""
        self.tables["departments"] = self.generate_departments()
        self.tables["salaries"] = self.generate_salaries()
        self.tables["employees"] = self.generate_employees()
        self.export_relational_csv()

    def generate_departments(self):
        """Generates department lookup table."""
        return [{"department_id": i, "department_name": name} for i, name in enumerate(["HR", "IT", "Finance", "Marketing"], start=1)]

    def generate_salaries(self):
        """Generates salary data with unique salary IDs."""
        return [
            {
                "salary_id": i,
            ...
            for i in range(1, self.config.num_rows // 5 + 1)
        ]

    def generate_employees(self):
        """Generates employees and assigns department & salary foreign keys."""
        return [
            {
                "employee_id": i,
                "full_name": self.fake.name(),
                "department_id": random.choice(self.tables["departments"])["department_id"],
                "salary_id": random.choice(self.tables["salaries"])["salary_id"]
            }
            for i in range(1, self.config.num_rows + 1)
        ]
...
if __name__ == "__main__":
    ...
    # Create a configuration for relational data
    hr_config = CSVConfig([100, 10, 0.05])  # 100 rows, 10 duplicates, 5% null probability

    # Generate HR relational dataset
    hr_database = FakeHRDatabase("fake_hr", hr_config)
    hr_database.generate_data()
